...
function cRMain(config, csvFile){
  // TODO: sum up features for connection
  // TODO: make outer circle optional
  var center = Array(Math.round(config.image_size / 2),
                     Math.round(config.image_size / 2));

  // margin originally was 20
  ...
  if (csvFile != null){
      ...
      if (typeof feature == "object"){
        ...
        var connection_threshold = feature[3]; // value under which the smaples are considered
                                               // similar.
        diagram_colors = setMode(feature[feature.length - 1]);
        setBgr(diagram_colors[0], config);
        ...
        TinyRad = standardize(TinyRad, tiny_rad);

        for (i=0; i< Countries.length; i++){
          var Text = config.doc.layers.getByName('Countries').textFrames.add();
          Text.contents = Countries[i];
          distribute_cr(Text, i, fileArray.length, config.radius - offset - (WLength / 2),
                        0, center);
          CWLength = Math.max(CWLength, Text.width);

          // if ((i % 5) == 0){
          if (fileArray[i+1][getIndex(Columns, feature[0])] != fileArray[i][getIndex(Columns, feature[0])]){
            var Text = config.doc.layers.getByName('Parts').textFrames.add();
            Text.contents = Parts[i];
            PartsIndices.push(Parts.length - i - 1);
            distribute_cr(Text, i, fileArray.length, config.radius, 0, center);
            WLength = Math.max(WLength, Text.width);
          }
        }
        PartsIndices = PartsIndices.reverse();
        if ((WLength + config.radius + config.margin) > (config.image_size / 2)){
          config.radius = Math.round((config.image_size / 2) - config.margin - WLength);
          center = Array(Math.round(config.image_size / 2),
                         Math.round(config.image_size / 2));
        }
        var parts_number = config.doc.layers.getByName('Parts').textFrames.length;
        var countries_number = config.doc.layers.getByName('Countries').textFrames.length;
        var inner_circle_radius = config.radius - offset -
                                  Math.round(CWLength + (offset / 2));
        for (t=0; t < countries_number; t++){

          var Text = config.doc.layers.getByName('Countries').textFrames[t];
          textRotate(Text, t-2, countries_number);
          textApp(Text, 14, getArrayFromColor(diagram_colors[1]));
          distribute_cr(Text, t-2, countries_number, inner_circle_radius, 0, center);
          ...
          var small_ellipse = config.doc.layers.getByName('Graphical_small').pathItems.ellipse(0, 0,
                                                     _tiny_rad * 2,
                                                     _tiny_rad * 2,
                                                     false);
          distribute_cr(small_ellipse, countries_number - t - 1, countries_number,
                     inner_circle_radius - offset, _tiny_rad, center);
          objApp(small_ellipse, getArrayFromColor(diagram_colors[1]));

          if (t < parts_number){
            var Text = config.doc.layers.getByName('Parts').textFrames[t];
            textRotate(Text, PartsIndices[t]-2, countries_number);
            textApp(Text, 14, getArrayFromColor(diagram_colors[1]));
            distribute_cr(Text, PartsIndices[t]-2, countries_number, config.radius, 0, center);

            // Add a circle near each Part
            var small_ellipse = config.doc.layers.getByName('Graphical').pathItems.ellipse(Math.round(center[0] + (small_rad / 2) +
                            ((config.radius - offset) * Math.cos((PartsIndices[t]-2) * (360/countries_number) * Math.PI / 180))),
                            Math.round(center[0] - (small_rad / 2) + ((config.radius - offset) *
      ...
      var large_ellipse = config.doc.layers.getByName('Graphical').pathItems.ellipse(center[0] + config.radius - offset,
                                                     center[1] - config.radius + offset,
                                                     (config.radius - offset) * 2,
                                                     (config.radius - offset) * 2, false);

      large_ellipse.filled = false;
      large_ellipse.strokeColor = diagram_colors[1];
      large_ellipse.zOrder(ZOrderMethod.SENDTOBACK);  // bring it to the back
      // Adding the countries as a second circle.
      Compare(GDP, connection_threshold);
    }
    else {
...
function distribute_cr(_obj, _i, _length, _radius, _own_radius, center){
...
////////////////////////////////////////////////////////////////////////////////
    ...
    /* Function that connects similar elements by feature.
       Input: _first         color values, array (ch, ch, ch [, ch])
              ...
              _note          special position of the elements, bool
                             ...
                             1 - invert the curve
    */
    var _ell1 = config.doc.layers.getByName('Graphical_small').pathItems[_first];
    var _ell2 = config.doc.layers.getByName('Graphical_small').pathItems[_second];
    _coords = [[(_ell1.left + (_ell1.width/2)),(_ell1.top - (_ell1.height/2))],
               [(_ell2.left + (_ell2.width/2)),(_ell2.top - (_ell2.height/2))]]
    var _line = config.doc.layers.getByName('Connections').pathItems.add();
  ...
  function Compare(_array, connection_threshold){
